...
class Import extends BaseController {
    ...
    public function index() {
        ...
        $rss_feed_url = env('strava.rss_feed_url');
        if (!$rss_feed_url) {
        ...
        foreach ($rss->channel->item as $item) {
            ...
            $existingStrava = $StravaModel->where('guid', $item->guid)->first();

            if (empty($existingStrava)) {
                ...
                $StravaModel->insert($data);
                $newStravasCount++;
                log_message('info', 'Inserted new Strava entry: ' . $title);

                // Insert the new entry into the main site stream table.
                $data = array(
                    'date' => date('Y-m-d H:i:s', strtotime($pubDate)),
                    'title' => $title,
                    'url' => str_replace([env('strava.rss_link_root'), '/'], ['', ''], $link), // strip the root URL and trailing slash
                    'source' => $link,
                    'excerpt' => $description,
                    'content' => $content,
                    'type' => 'strava',
                ...
                log_message('info', 'Inserted new Strava entry into main stream table.');
